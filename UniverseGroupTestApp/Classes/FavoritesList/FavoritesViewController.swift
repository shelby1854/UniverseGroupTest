...
final class FavoritesViewController: UIViewController {
  ...
  private func setupBindings() {
    viewModel.output.favoriteFilms
    ...
    viewModel.output.isPlaceholderHidden
    ...
    tableView.rx.itemDeleted
      .withLatestFrom(viewModel.output.favoriteFilms) { (indexPath, items) -> FilmBO? in
  ...
  @objc private func removeSelectedTapped() {
    ...
    alert.addAction(UIAlertAction(title: alertTexts.confirm, style: .destructive, handler: { [weak self] _ in
      guard let self else { return }
      self.viewModel.input.removeSelected.onNext(()) 
      self.tableView.reloadData()
    }))
